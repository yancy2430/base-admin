<template>
  <a-form layout="inline"
    ...
    <a-row type="flex" :gutter="48" :key="searchVersion">
      <a-col :md="8" :sm="24" v-for="(item,index) in value" :key="index" v-if="!advanced?index<2:true">
        <a-form-item :label="item.label">
          <a-select v-if="item.input!=7 && !item.enumHash" @change="selectChange($event,item)" v-model="item.select"
                    style="width: 24%" :options="item.finds">
          </a-select>
          <!--基本输入框-->
          <a-input v-if="item.input==1" v-model="item.value" style="width: 76%;margin-left: -3px;" placeholder=""/>
          <!--数值/区间-->
          <span v-else-if="item.input==2">
                    <between-input v-if="item.input==2 && [7,8].indexOf(item.select)!=-1" v-model="item.value"
                                   style="width: 76%;margin-left: -1px;">
                    </between-input>
                    <a-input-number v-if="item.input==2 && [7,8].indexOf(item.select)==-1"
                                    style="width: 76%;margin-left: -2px;" v-model="item.value"/>
                  </span>
          ...
          <!--日期-->
          <a-date-picker valueFormat="YYYY-MM-DD" v-else-if="item.input==5 && [7,8].indexOf(item.select)==-1"
                         v-model="item.value" style=" width: 76%;margin-left: -3px;"/>
          <!--日期区间-->
          <a-range-picker valueFormat="YYYY-MM-DD" v-else-if="item.input==5 && [7,8].indexOf(item.select)!=-1"
                          v-model="item.value" style=" width: 76%;margin-left: -3px;"/>
          <!--日期时间-->
          <a-date-picker valueFormat="YYYY-MM-DD HH:mm:ss" show-time
                         v-else-if="item.input==6 && [7,8].indexOf(item.select)==-1" v-model="item.value"
                         style=" width: 76%;margin-left: -3px;"/>
          <!--日期时间区间-->
          <a-range-picker @change="selectChange($event,item)" valueFormat="YYYY-MM-DD HH:mm:ss" show-time
                          v-else-if="item.input==6 && [7,8].indexOf(item.select)!=-1" v-model="item.value"
                          style=" width: 76%;margin-left: -3px;"/>
